...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      "Analytics API: Found organizationId:",
      organizationId,
      "for organization:",
    ...
    const dailyActivityMap = new Map<
      string,
      { date: string; count: number; genuine: number; suspicious: number }
    >();
    ...
    dailyScans.forEach((scan: { scanDate: Date; scanResult: string }) => {
      ...
      const dayData = dailyActivityMap.get(dateStr)! as DailyActivityData;
      dayData.count++;
      if (scan.scanResult === "GENUINE") dayData.genuine++;
      if (scan.scanResult === "SUSPICIOUS") dayData.suspicious++;
    });

    const processedDailyActivity = Array.from(dailyActivityMap.values()).sort(
      ...
      try {
        ...
        const [
          ...
          prisma.scanHistory.count({
            ...
            prisma.medicationBatch
            .groupBy({
              by: ["drugName"],
              where: { organizationId },
              _count: { drugName: true },
            })
            .then((result: Array<{ drugName: string; _count: { drugName: number } }>) => result.length),
